...
class RPC:
    ...
    def create_feedback(self, project_id: int, clean_data: dict) -> dict:
        if not clean_data.get('user_id'):
            clean_data['user_id'] = auth.current_user().get("id")
        try:
            feedback_data = FeedbackModel(**clean_data)
        except ValidationError as e:
            return {'ok': False, 'error': str(e)}

        try:
            feedback = Feedback(project_id=project_id, **feedback_data.dict())
            feedback.insert()
        except IntegrityError as e:
            return {"ok": False, "error": "You have already given a feedback to this page"}
        except Exception as e:
            return {"ok": False, "error": str(e)}
        return {'ok': True, 'result': feedback}
    

    @web.rpc("social_list_feedbacks", "get_feedbacks")
    def list_feedbacks(self, project_id: int | None, args: dict) -> dict:
